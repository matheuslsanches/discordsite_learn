  ...
  function onlyCreateElement(element) {
    return {
      ...
      build() {
        ...
        if (this.elementByTimes.length > 0) return this.elementByTimes;
...
function factoryBuilder(HTMLContentNative, body) {
  function addUtils({ toInside }) {
    return {
      ...
      insideOf(component) {
        if (!component) return body.appendChild(toInside);
  ...
  return Object.defineProperty(HTMLContentNative, "build", {
    ...
    writable: false,
    value(node, callback) {
      const allContentItems = Object.values(HTMLContentNative).filter(
...
function styleSheet({ body, create }) {
  ...
  // Compose elements
  header
    .build(myHeader, (...content) => {
      return onSelector.add(content, body).header();
    })
    .mount("header")
    .insideOf(body.component);

  navigation
    .build(myMenu, (...content) => {
      return onSelector.add(content, body).navigation();
    })
    .mount("navigation")
    .insideOf(header.component);

  article
    .build(myArticle, (...content) => {
      ...
      .then((data) => {
        data
          ...
          .then((text) => {
            ...
            const removeScript = text.substr(
              text.search("<!-- Code injected by live-server -->"),
              text.search("</script>")
            );
    ...
    value([node, content, next], body) {
      return {
        ...
        article() {
          ...
          // Filtered
          const image = content.item.filter(
            (value) => value.nodeName === "IMG"
          );
          const title = content.item.filter((value) => value.nodeName === "H2");
          const description = content.item.filter(
            (value) => value.nodeName === "P"
          );
          ...
          moldes.create("ltr", "rtl").to(3).document(sections).build();

          moldes
            .create(null, "bg-section")
            .to(4)
            .document(sections)
            .build((state, child, current) => {
              if (current >= 4) child.classList.add("section-extended-mode");
              return state();
            });
        ...
        download() {
          ...
          content.item.forEach((item) => {
            if (item.nodeName == "IMG") return sparkles.appendChild(item);
        ...
        footer() {
          const [footer, rows, routes, language, social, container] = node;
